...
// Raw SQL columns: Image, Asset Name, Asset Link, Campaign, Impressions, Clicks, CTR
...
function GatedContentTable({ campaign, dateStart, dateEnd, channel }: { campaign: string[]; dateStart: string; dateEnd: string; channel: string[] }) {
  ...
  // Channel narrows the underlying rows server-side (in q205-data's SQL
  // WHERE clause, before aggregation) — the table stays one row per asset,
  // summed across whichever channels are selected.
